...
void inGame() {
	...
	int turn = 0; 
	bool QUIT = false;
	while(isShipAlive) { // 유저가 플레이하는 부분 
		system("cls");
		...
		printf(" O : 맞춤, X : 빗나감");
		gotoxy(0, remark);
		printf("정수만 입력해주세요. (0 입력 시 게임 종료, -1 입력 시 정답 보기)", map_size-2, map_size-2);
		...
		if (flag == 0) {
			...
			printf("!!!!!!!!!!VICTORY!!!!!!!!!!");
			gotoxy(33, 8);
			printf("%d발 만에 적함을 모두 격침!", turn);
			for (int i=0; i<map_size; i++) { 
				gotoxy(46-map_size, 10+i);
				for (int j=0; j<map_size; j++) {
					printf("~ ");
				}
					printf("\n");
	...
	printf("[ 게임 종료 ]");
	gotoxy(0, remark);
	printf("SPACEBAR를 누르면 타이틀 화면으로 돌아갑니다.");
...
void Manual() {
	...
	gotoxy(16, 14);
	printf("좌표에 0을 입력하면 게임 종료, -1을 입력하면 답을 보여줍니다.");
	gotoxy(30, 17);
